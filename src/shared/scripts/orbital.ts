...
/**
 * Velocity-based scroll inertia: each card shears (skewX) proportionally to
 * scroll velocity, propagated as a staggered wave, then springs back to 0.
 *
 * rotationX was removed: it is a 3D transform that promotes each card to its
 * own GPU compositing layer rasterised at the card's current (small) size.
 * When the ring is scaled ×3 by scroll that cached texture stretches and looks
 * pixelated — and the artefact persists even after the ring returns to scale 1.
 * skewX is a 2D transform and re-rasterises at screen resolution every frame.
 *
...
const addScrollInertia = (thumbs: HTMLElement[]): (() => void) => {
  ...
  const springBack = () => {
    gsap.killTweensOf(thumbs, "skewX");
    gsap.to(thumbs, {
      skewX: 0,
      duration: 0.8,
      stagger: 0.02,
  ...
  const onScroll = () => {
    ...
    const velocity = now - lastScrollY;
    lastScrollY = now;
