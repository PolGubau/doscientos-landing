...
export async function getBrandKit(): Promise<BrandKit> {
  ...
  try {
    ...
    const guides = Array.isArray(payload.guides) ? payload.guides.filter(isBrandGuide) : []
    const tokens = Array.isArray(payload.tokens) ? payload.tokens.filter(isBrandToken) : []
    return {
      guides:
        guides.length > 0 ? guides.sort((a, b) => a.sort_order - b.sort_order) : fallbackGuides,
      tokens:
        tokens.length > 0 ? tokens.sort((a, b) => a.sort_order - b.sort_order) : fallbackTokens,
    }
  } catch {
